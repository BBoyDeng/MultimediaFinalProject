...
public class Time_Controller_3X3X3 : MonoBehaviour {
	...
	void RandomCreateCube(){
		...
		isMoved = false;
		if (cur_cubeNum == numOfOBJ-3 && isGameOver ()) {
			lightRotateX = 0f;
			gameover = true;
			Debug.Log ("GameOver");
		}
	...
	bool isGameOver (){
		...
		for (int i = numOfOBJ-2; i >= 1; i -= LENGTH * LENGTH)
		{
			for (int j = i; j >= i-LENGTH ; j-=LENGTH)
			{
		...
		for (int i = numOfOBJ - 6; i >= 1; i -= LENGTH * LENGTH)
		{
			for (int j = i; j >= i-LENGTH+2; j--)
			{
				if (score[j] == score[j - 1] && score[j] != 0)
		...
		for (int i = numOfOBJ; i >= numOfOBJ-LENGTH+1; i --)
		{
			for (int j = i; j > LENGTH*LENGTH ; j -= LENGTH*LENGTH)
			{
				if (score[j] == score[j - LENGTH * LENGTH] && score[j] != 0)
				{
					return false;
		...
		for (int i = numOfOBJ-2; i >= numOfOBJ-2 - LENGTH*2; i-=LENGTH)
		{
			for (int j = i; j > LENGTH * LENGTH; j -= LENGTH * LENGTH)
			{
				if (score[j] == score[j - LENGTH * LENGTH] && score[j] != 0)
		...
		for (int i = numOfOBJ-6; i >= numOfOBJ - LENGTH + 1; i--)
		{
			for (int j = i; j > LENGTH * LENGTH; j -= LENGTH * LENGTH)
			{
				if (score[j] == score[j - LENGTH * LENGTH] && score[j] != 0)
